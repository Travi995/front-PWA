...
const CompCategory:FC<compCategoryProps> = ({icon, handlerChange, handlerClick=()=>{}}, active ) => {
    ...
    return <div onClick={handlerEvent} className="relative flex flex-col items-center">
        <div onClick={handlerEvent} className={`bg-gray-400 transition-all duration-200 w-20 h-20 rounded-full flex items-center justify-center shadow hover:cursor-pointer ${active?'bg-gray-500':'bg-gray-400'} `}>
            <i className={`pi ${icon}`} 
